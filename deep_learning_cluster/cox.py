...
class _CoxBase(base.SurvBase):
    ...
    def fit(self, x1,x2, target, batch_size=256, epochs=1, callbacks=None, verbose=True,
        ...
        """
        self.training_data = tt.tuplefy(x1,x2, target)
        print(metrics)
        return super().fit(x1,x2, target, batch_size, epochs, callbacks, verbose,
                           num_workers, shuffle, metrics, val_data, val_batch_size,
                           **kwargs)

    def _compute_baseline_hazards(self, x1,x2, df, max_duration, batch_size, eval_=True, num_workers=0):
    ...
    def compute_baseline_hazards(self, x1=None, x2=None,target=None, max_duration=None, sample=None, batch_size=8224,
        ...
        Typically call
        model.compute_baseline_hazards()compute_baseline_hazards() after fitting.
        ...
        """
        if (x1 is None) and (x2 is None) and (target is None):
            if not hasattr(self, 'training_data'):
                raise ValueError("Need to give a 'input' and 'target' to this function.")
            x1,x2, target = self.training_data
        ...
        x1 = tt.tuplefy(x1).to_numpy().iloc[df.index.values].astype('float')
        x2 = tt.tuplefy(x2).to_numpy().iloc[df.index.values].astype('float')
        #x1=tuple_of_tensors_to_tensor(x1).float()
        #x2=tuple_of_tensors_to_tensor(x2).float()

        base_haz = self._compute_baseline_hazards(x1,x2, df, max_duration, batch_size,
                                                  eval_=eval_, num_workers=num_workers)
        if set_hazards:
            self.compute_baseline_cumulative_hazards(set_hazards=True, baseline_hazards_=base_haz)
        return base_haz
    def compute_risk(self, x1=None, x2=None,target=None, max_duration=None, sample=None, batch_size=8224,
                                 set_risk=True, eval_=True, num_workers=0):
        """Computes the Breslow estimates form the data defined by `input` and `target`
        (if `None` use training data).
        Typically call
        model.compute_baseline_hazards()compute_baseline_hazards() after fitting.
        ...
        risk = self._compute_risk(x1,x2, df, max_duration, batch_size,
                                                  eval_=eval_, num_workers=num_workers)
        if set_risk:
            self.compute_risk(set_risk=True, baseline_hazards_=risk)
        return risk


    def compute_baseline_cumulative_hazards(self, x1=None, x2=None,target=None, max_duration=None, sample=None,
        ...
        if ((x1 is not None) or (target is not None) or (x2 is not None)) and (baseline_hazards_ is not None):
            raise ValueError("'input', 'target' and 'baseline_hazards_' can not both be different from 'None'.")
        if baseline_hazards_ is None:
            baseline_hazards_ = self.compute_baseline_hazards(x1,x2, target, max_duration, sample, batch_size,
    ...
    def predict_cumulative_hazards(self, x1,x2, max_duration=None, batch_size=8224, verbose=False,
                                   baseline_hazards_=None, eval_=True, num_workers=0):
        """See `predict_survival_function`."""
        if type(x1) is pd.DataFrame:
            x1 = self.df_to_input(x1)
        if type(x2) is pd.DataFrame:
            x2 = self.df_to_input(x2)
        if baseline_hazards_ is None:
        ...
        assert baseline_hazards_.index.is_monotonic_increasing, \
            'Need index of baseline_hazards_ to be monotonic increasing, as it represents time.'
        return self._predict_cumulative_hazards(x1,x2, max_duration, batch_size, verbose, baseline_hazards_,
                                                eval_, num_workers=num_workers)

    def _predict_cumulative_hazards(self, x1,x2, max_duration, batch_size, verbose, baseline_hazards_,
                                    eval_=True, num_workers=0):
        raise NotImplementedError

    def predict_surv_df(self, x1,x2, max_duration=None, batch_size=8224, verbose=False, baseline_hazards_=None,
                        eval_=True, num_workers=0):
        ...
        """
        return np.exp(-self.predict_cumulative_hazards(x1,x2, max_duration, batch_size, verbose, baseline_hazards_,
                                                       eval_, num_workers))

    def predict_surv(self, x1,x2, max_duration=None, batch_size=8224, numpy=None, verbose=False,
                     baseline_hazards_=None, eval_=True, num_workers=0):
        """Predict survival function for `input`. S(x, t) = exp(-H(x, t))
        ...
        Returns:
            pd.DataFrame -- Survival estimates. One columns for each individual.
        """
        surv = self.predict_surv_df(x1,x2, max_duration, batch_size, verbose, baseline_hazards_,
                                    eval_, num_workers)
        surv = torch.from_numpy(surv.values.transpose())
        return torchtuples.utils.array_or_tensor(surv, numpy, x1)
...
class _CoxPHBase(_CoxBase):
    def _compute_baseline_hazards(self, x1,x2, df_target, max_duration, batch_size, eval_=True, num_workers=0):
        ...
        return (df_target
                .assign(expg=np.exp(np.squeeze(self.predict(x1,x2, batch_size, True, eval_, num_workers=num_workers)[0],axis=0)+
                                    self.predict(x1,x2, batch_size, True, eval_, num_workers=num_workers)[1]))
                #.assign(expg=np.exp(self.predict(x1, x2, batch_size, True, eval_, num_workers=num_workers),dtype=float))
    ...
    def _compute_risk(self, x1,x2, df_target, max_duration, batch_size, eval_=True, num_workers=0):
        if max_duration is None:
            max_duration = np.inf
        ...
        #   Could be made faster, by only computing when there are events.
        return (df_target
                .assign(expg=(np.squeeze(self.predict(x1,x2, batch_size, True, eval_, num_workers=num_workers)[0],axis=0)+
                                    self.predict(x1,x2, batch_size, True, eval_, num_workers=num_workers)[1]))
                #.assign(expg=np.exp(self.predict(x1, x2, batch_size, True, eval_, num_workers=num_workers),dtype=float))
                .rename('risk'))
    ...
    def _predict_cumulative_hazards(self, x1,x2, max_duration, batch_size, verbose, baseline_hazards_,
        ...
        expg = np.exp(np.squeeze(self.predict(x1,x2, batch_size, True, eval_, num_workers=num_workers)[0],axis=0)+
                      self.predict(x1,x2, batch_size, True, eval_, num_workers=num_workers)[1]).reshape(1, -1)
        return pd.DataFrame(bch.values.reshape(-1, 1).dot(expg),
                            index=bch.index)

    def partial_log_likelihood(self, x1,x2, target, g_preds=None, batch_size=8224, eps=1e-7, eval_=True,
                               num_workers=0):
        ...
        if g_preds is None:
            g_preds = self.predict(x1,x2, batch_size, True, eval_, num_workers=num_workers)
            g_preds=np.squeeze(g_preds[0],axis=0)+g_preds[1]
        #return g_preds
        return (df
