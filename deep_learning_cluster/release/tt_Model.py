...
class Model(object):
    ...
    def _setup_train_info(self, dataloader):
        ...
        if data is not None:
            if len(data) == 3:
                try:
                    x1,x2,target = data
                    self.fit_info["x1"] = _tuple_info(x1)
                    self.fit_info["x2"] = _tuple_info(x2)
                except:
                    pass
        ...
        if metrics is not None:
            if not hasattr(metrics, "items"):
                if not hasattr(metrics, "__iter__"):
                    metrics = [metrics]
                metrics = {met.__ne__: met for met in metrics}
            if "loss" in metrics:
        ...
        with torch.no_grad():
            for data in dataloader:
                if score_func is None:
                    ...
                    x1,x2, target = data
                    score = score_func(self, x1,x2, target)
    ...
    def _predict_func_dl(
        ...
        if hasattr(self, "fit_info") and (self.make_dataloader is self.make_dataloader_predict):
            ...
            if data is not None:
                ...
                x1 = tuplefy(data[0])
                x1_train = self.fit_info.get("x1")
                x2 = tuplefy(data[1])
                x2_train = self.fit_info.get("x2")

                if x1_train is not None:
                    if x1.to_levels() != x1_train.get("levels"):
                        warnings.warn(
                    ...
                    if x1.shapes().apply(lambda x: x[1:]) != x1_train.get("shapes"):
                        warnings.warn(
                            """The input from the dataloader is different from
                        the 'input' during trainig. The shapes are different."""
                        )
                if x2_train is not None:
                    if x2.to_levels() != x2_train.get("levels"):
                        warnings.warn(
                            """The input from the dataloader is different from
                        the 'input' during trainig. Make sure to remove 'target' from dataloader.
                        Can be done with 'torchtuples.data.dataloader_input_only'."""
                        )
                    if x2.shapes().apply(lambda x: x[1:]) != x2_train.get("shapes"):
                        warnings.warn(
                            """The input from the dataloader is different from
                        the 'input' during trainig. The shapes are different."""
                        )
        ...
        with torch.set_grad_enabled(grads):
            ...
            for input in dataloader:
                x1 = tuplefy(input[0]).to_device(self.device)
                x2 = tuplefy(input[1]).to_device(self.device)
                preds_batch = tuplefy(func(x1,x2))
                if numpy or to_cpu:
    ...
    def _predict_func(
        ...
        **kwargs,
        ...
        return array_or_tensor(preds, numpy, x1)
...
def _get_element_in_dataloader(dataloader):
    ...
    try:
        ...
        pass
    try:
        return dataset[[0, 1,2]]
    except:
        pass
    try:
        return dataloader.collate_fn([dataset[0], dataset[1],dataset[2]])
    except:
        pass
    return None
